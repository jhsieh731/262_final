...
class ClientApp:
    def __init__(self, root):
        ...
        self.status_var.set("Finding leaders...")
    ...
    def _start_heartbeat_threads(self):
        ...
        def monitor(stubs, update_leader_fn, service_name=""):
            while True:
                try:
                    leader = None
                    ...
                    if leader:
                        ...
                        self.root.after(0, lambda s=service_name: self.status_var.set(f"No {s} leader found"))
    ...
    def login(self):
        ...
        def login_to_shard1():
            ...
            if not stub:
                # Try shard 2 if shard 1 leader not available
                login_to_shard2()
                return
            ...
            def on_success(response):
                if response.success:
                    ...
                    logger.info(f"User {username} logged in with ID {self.user_id} on shard 1")
        ...
        # If shard 1 fails, try shard 2
        def login_to_shard2():
            stub = self.get_shard2_stub()
            if not stub:
                messagebox.showerror("Error", "No leader available for either shard")
                self.login_result_var.set("No leader available")
                return
            ...
            def on_success(response):
                if response.success:
                    self.user_id = response.user_id
                    self.current_username = username
                    self.current_shard = 2
                    logger.info(f"User {username} logged in with ID {self.user_id} on shard 2")
                    self.login_result_var.set("Login successful")
                    self.load_cart()
                    self.switch_to_main()
                else:
                    self.login_result_var.set("User not found")
    ...
    def create_account(self):
        ...
        if not stub:
            ...
            self.register_result_var.set("No leader available")
            return
        ...
        def on_success(response):
            if response.success:
                ...
                logger.info(f"Account created for {username} with ID {self.user_id} on shard {target_shard}")
    ...
    def load_cart(self):
        ...
        if not stub:
            logger.warning("No leader available for cart loading")
            return
        ...
        def make_rpc():
            return stub.GetCart(user_cart_pb2.UserRequest(user_id=self.user_id))
    ...
    def checkout(self):
        ...
        # We need to perform multiple RPCs in sequence
    ...
    def get_user_cart_stub(self):
        """Get the appropriate stub based on user's shard"""
